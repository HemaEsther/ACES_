const ClassicTemplate = ({ details }) => (
    <div className="p-6 border border-gray-500 shadow-md bg-white max-w-[800px] mx-auto font-serif">
      {/* Header */}
      <div className="text-center border-b-2 pb-3">
        <h1 className="text-3xl font-bold">{details.name || "Your Name"}</h1>
        <p className="text-gray-700">{details.email || "your.email@example.com"} | {details.phone || "123-456-7890"}</p>
      ...
      <div className="mt-5">
        ...
        <p className="italic">{details.company || "Company Name"} | {details.duration || "2022 - Present"}</p>
      ...
      <div className="mt-5">
        ...
        <p className="italic">{details.university || "XYZ University"} | {details.year || "2018 - 2022"}</p>
      </div>
  
      {/* Skills */}
      <div className="mt-5">
        <h2 className="text-lg font-semibold border-b pb-1">Technical Skills</h2>
        <p>{details.skills || "React, Redux, Node.js, Express, MongoDB, Git"}</p>
      </div>
    </div>
  );
